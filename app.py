    ...
    if form.validate_on_submit():  # 내용 채우지 않은 항목이 있는지까지 체크
        ...
        if not userdata:  # 가입된 ID가 없을 경우
            ...
            done_user = db.session.query(DoneTeamData).\
                ...
                filter(UserData.userId==userid).first()  # 만족 여부가 저장되어 있지 않은 유저가 속한 팀 조회Query 실행
            if done_user:  # 쿼리 데이터가 존재할 경우
                return redirect('/satisfy')
            else:  # 유저가 속한 팀이 없거나, 속한 팀에 대한 만족 여부 데이터가 저장되어 있는 경우
...
def ask_satisfy():
    ...
    if form.validate_on_submit():  # 유효할 경우
        usersat = db.session.query(DoneTeamData).filter(DoneTeamData.userNum == UserData.userNum, DoneTeamData.userSat == None).\
            filter(UserData.userId == userid).first()   # 만족 여부가 저장되어 있지 않은 유저가 속한 팀 조회Query 실행
        value = dict(form.input_sat.choices).get(form.input_sat.data)
        usersat.userSat = value # form에서의 input_sat을 가져와 usersat의 만족 여부에 저장
...
def sendCondition():
    ...
    # 여행 목적지 선택
    form.travelDes.choices = [(a.countryName, a.countryName) for a in NeedLangData.query.group_by(NeedLangData.countryLang)]
    # 특기 언어 선택
    form.travelLang.choices = [(a.countryLang, a.countryLang) for a in NeedLangData.query.group_by(NeedLangData.countryLang)]

    if form.validate_on_submit():  # 유효할 경우
        conditiondata = ConditionData()
        conditiondata.travelDes = form.data.get('travelDes')  # form에서의 travelDes 데이터를 가져와 ConditionData 클래스의 travelDes 저장
        value = dict(form.travelNum.choices).get(form.travelNum.data)
        conditiondata.travelNum = value # form에서의 travelNum 데이터를 가져와 ConditionData 클래스의 travelNum 저장
...
# teamcreate 페이지 접속(GET)처리와, "action=/teamcreate" 처리(POST)모두 정의
@app.route('/teamcreate', methods=['GET', 'POST'])
def inserWaitTeamData():
    userid = session.get('userId', None)
    userdata = UserData.query.filter(UserData.userId == userid).first()
    form = CreateTeam() # 팀 생성 폼
    form.teamTo.choices = [(a.countryName, a.countryName) for a in NeedLangData.query.group_by(NeedLangData.countryName)] # NeedLangData 테이블에서 나라 이름을 가져와 그 중 선택할 수 있게 함
    ...
    if form.validate_on_submit():   # 유효할 경우
        ...
        value = dict(form.teamNumGoal.choices).get(form.teamNumGoal.data)
        waitteamdata.teamNumGoal = value # form에서의 teamNumGoal 데이터를 가져와 waitteamdata 클래스의 teamNumGoal 저장
        ...
        flash('팀생성이 완료되었습니다.')
        return redirect('/')
    return render_template('teamcreate.html', form=form, userid=userid, userdata=userdata)
